...
export function Footer() {
    ...
    return (
        <footer className="w-full mt-20 sm:mt-32 py-12 border-t border-border/10 flex flex-col md:flex-row justify-between items-center gap-8 text-[10px] uppercase tracking-widest text-muted-foreground/80 transition-colors hover:text-muted-foreground">
            ...
            <div className="flex items-center gap-8 order-2 md:order-1">
                {photographer?.socials?.instagram && (
                    <a
                        ...
                        className="hover:text-foreground transition-all duration-300 hover:scale-110 p-2 -m-2 opacity-80 hover:opacity-100"
            ...
            <div className="flex flex-col md:flex-row items-center gap-4 md:gap-8 order-1 md:order-2 text-center md:text-right">
                ...
                <div className="flex flex-col md:flex-row items-center gap-2 md:gap-6">
                    <span className="font-serif text-foreground tracking-widest text-xs">
                        {t(photographer?.name || 'Photographer')}
                    </span>
                    <span className="hidden md:inline text-border/40">|</span>
                    <span className="opacity-80 font-medium">© {currentYear} {t('footer.rights_reserved')}</span>
                </div>
                ...
                <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="group relative flex items-center gap-1 hover:text-foreground transition-colors opacity-80 hover:opacity-100"
                >
