          ...
          {images.map((image, index) => (
            ...
            >
              ...
              <div className="absolute bottom-4 left-4 right-4">
                <p className="text-sm text-primary-foreground/90 font-medium">{image.code}</p>
              </div>
            </motion.div>
          ))}
        </div>
                ...
                <AnimatePresence>
                  {isActive && (
                    <motion.div
                      ...
                      animate={{ opacity: 1 }}
                      exit={{ opacity: 0 }}
                      className="absolute h-full w-full bg-gradient-to-t from-foreground/40 to-transparent z-10"
                    />
                  )}
                </AnimatePresence>
                <AnimatePresence>
                  {isActive && (
                    ...
                    >
                      <p className="text-left text-xs text-primary-foreground/50">{image.code}</p>
                    </motion.div>
                  )}
                </AnimatePresence>
                <img src={image.src} className="size-full object-cover" alt={image.alt} />
